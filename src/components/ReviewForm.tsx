...
import { SliceReview, SelfRating } from '../types';
...
export const ReviewForm = () => {
  ...
  const [sliceReviews, setSliceReviews] = useState<Record<string, Partial<SliceReview>>>(() => {
    const initial: Record<string, Partial<SliceReview>> = {};
  ...
  const totalActualDuration = Object.values(sliceReviews).reduce(
    (sum, r) => sum + (r.actualDurationMinutes || 0), 0
  );
  ...
  const handleSubmit = () => {
    const validReviews = activeSlices
      .map((slice) => sliceReviews[slice.id])
      .filter((r): r is SliceReview =>
        r.sliceId !== undefined &&
        r.actualDurationMinutes !== undefined &&
        r.isStuck !== undefined &&
        r.selfRating !== undefined
      );
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-slide-up">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">记录本次排练的问题和改进建议</p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-3 gap-4 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-indigo-600">{activeSlices.length} 个</p>
            </div>
          ...
          <div className="space-y-3">
            ...
            {activeSlices.map((slice, index) => {
              const review = sliceReviews[slice.id] as Partial<SliceReview> & { sliceId: string };
              const isExpanded = expandedSliceId === slice.id;
              const hasIssues = review.isStuck ||
                (review.actualDurationMinutes || 0) > slice.durationMinutes ||
                (review.selfRating || 3) <= 2;

              return (
                <div
                  key={slice.id}
                  className={`rounded-xl border-2 transition-all ${
                    hasIssues ? 'border-amber-300 bg-amber-50/50' : 'border-slate-200 bg-white'
                  }`}
                >
                  <div
                  ...
                  >
                    <span className="w-8 h-8 flex items-center justify-center bg-slate-100 rounded-lg text-sm font-semibold text-slate-600">
                      {index + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className="font-semibold text-slate-800 truncate">{slice.title}</h4>
                        {hasIssues && (
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        预计 {slice.durationMinutes} 分钟 · 自评 {review.selfRating || 3}/5
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                    ...
                    <div className="px-4 pb-4 space-y-4 border-t border-slate-100 pt-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-32 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        />
                        {(review.actualDurationMinutes || 0) > slice.durationMinutes && (
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl">
          ...
          <button
            ...
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition-colors flex items-center gap-2 shadow-lg shadow-indigo-500/30"
          >
